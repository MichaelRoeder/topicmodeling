...
public class WikipediaMarkupDeletingMachine implements TextMachineObserver {
    ...
    public WikipediaMarkupDeletingMachine() {
    ...
    public void foundPattern(int patternId, int startPos, int endPos,
        ...
        case 22: {
            // this is the text of a single link which should be cleaned from markups
            String linkText = state.originalText.substring(startPos, endPos - 1);
            state.cleanText.append(getCleanString(linkText));
            break;
        }
        case 24: {
            // this is the text of a single link which should be cleaned from markups
            String linkText = state.originalText.substring(startPos, endPos);
            state.cleanText.append(getCleanString(linkText));
            break;
        }
        }
