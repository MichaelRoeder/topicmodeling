...
public class TopByteFloatCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopByteFloatCollectionDescending() {
        Random rand = new Random(System.currentTimeMillis());
        byte values[] = new byte[DATA_SIZE];
        float objects[] = new float[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
            values[i] = (byte)(rand.nextInt() >> 24);
            objects[i] = rand.nextFloat();
        }

        TopByteFloatCollection topCollection = new TopByteFloatCollection(TOP_X, false);
    ...
    @Test
    public void testTopByteFloatCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        byte values[] = new byte[DATA_SIZE];
        float objects[] = new float[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopByteFloatCollection topCollection = new TopByteFloatCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
