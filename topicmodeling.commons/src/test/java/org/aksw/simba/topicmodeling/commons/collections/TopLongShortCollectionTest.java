...
public class TopLongShortCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopLongShortCollectionDescending() {
        ...
        TopLongShortCollection topCollection = new TopLongShortCollection(TOP_X, false);
    ...
    @Test
    public void testTopLongShortCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        long values[] = new long[DATA_SIZE];
        short objects[] = new short[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopLongShortCollection topCollection = new TopLongShortCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
