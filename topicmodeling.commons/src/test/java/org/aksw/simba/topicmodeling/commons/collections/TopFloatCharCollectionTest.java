...
public class TopFloatCharCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopFloatCharCollectionDescending() {
        ...
        TopFloatCharCollection topCollection = new TopFloatCharCollection(TOP_X, false);
    ...
    @Test
    public void testTopFloatCharCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        float values[] = new float[DATA_SIZE];
        char objects[] = new char[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopFloatCharCollection topCollection = new TopFloatCharCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
