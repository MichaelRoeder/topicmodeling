...
public class TopCharIntCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopCharIntCollectionDescending() {
        ...
        TopCharIntCollection topCollection = new TopCharIntCollection(TOP_X, false);
    ...
    @Test
    public void testTopCharIntCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        char values[] = new char[DATA_SIZE];
        int objects[] = new int[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopCharIntCollection topCollection = new TopCharIntCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
