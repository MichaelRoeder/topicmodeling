...
public class TopFloatBooleanCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopFloatBooleanCollectionDescending() {
        ...
        TopFloatBooleanCollection topCollection = new TopFloatBooleanCollection(TOP_X, false);
    ...
    @Test
    public void testTopFloatBooleanCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        float values[] = new float[DATA_SIZE];
        boolean objects[] = new boolean[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopFloatBooleanCollection topCollection = new TopFloatBooleanCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
