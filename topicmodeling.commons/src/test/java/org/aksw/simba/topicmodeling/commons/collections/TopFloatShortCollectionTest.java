...
public class TopFloatShortCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopFloatShortCollectionDescending() {
        ...
        TopFloatShortCollection topCollection = new TopFloatShortCollection(TOP_X, false);
    ...
    @Test
    public void testTopFloatShortCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        float values[] = new float[DATA_SIZE];
        short objects[] = new short[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopFloatShortCollection topCollection = new TopFloatShortCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
