...
public class TopLongFloatCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopLongFloatCollectionDescending() {
        ...
        TopLongFloatCollection topCollection = new TopLongFloatCollection(TOP_X, false);
    ...
    @Test
    public void testTopLongFloatCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        long values[] = new long[DATA_SIZE];
        float objects[] = new float[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopLongFloatCollection topCollection = new TopLongFloatCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
