...
public class TopByteByteCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopByteByteCollectionDescending() {
        ...
        TopByteByteCollection topCollection = new TopByteByteCollection(TOP_X, false);
    ...
    @Test
    public void testTopByteByteCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        byte values[] = new byte[DATA_SIZE];
        byte objects[] = new byte[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopByteByteCollection topCollection = new TopByteByteCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
