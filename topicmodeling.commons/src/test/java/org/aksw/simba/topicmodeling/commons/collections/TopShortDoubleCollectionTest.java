...
public class TopShortDoubleCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopShortDoubleCollectionDescending() {
        ...
        TopShortDoubleCollection topCollection = new TopShortDoubleCollection(TOP_X, false);
    ...
    @Test
    public void testTopShortDoubleCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        double objects[] = new double[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopShortDoubleCollection topCollection = new TopShortDoubleCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
