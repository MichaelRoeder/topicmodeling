...
public class TopLongByteCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopLongByteCollectionDescending() {
        ...
        TopLongByteCollection topCollection = new TopLongByteCollection(TOP_X, false);
    ...
    @Test
    public void testTopLongByteCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        long values[] = new long[DATA_SIZE];
        byte objects[] = new byte[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopLongByteCollection topCollection = new TopLongByteCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
