...
public class TopCharLongCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopCharLongCollectionDescending() {
        ...
        TopCharLongCollection topCollection = new TopCharLongCollection(TOP_X, false);
    ...
    @Test
    public void testTopCharLongCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        char values[] = new char[DATA_SIZE];
        long objects[] = new long[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopCharLongCollection topCollection = new TopCharLongCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
