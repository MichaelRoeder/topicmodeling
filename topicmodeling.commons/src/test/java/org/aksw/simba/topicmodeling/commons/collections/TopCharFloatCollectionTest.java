...
public class TopCharFloatCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopCharFloatCollectionDescending() {
        ...
        TopCharFloatCollection topCollection = new TopCharFloatCollection(TOP_X, false);
    ...
    @Test
    public void testTopCharFloatCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        char values[] = new char[DATA_SIZE];
        float objects[] = new float[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopCharFloatCollection topCollection = new TopCharFloatCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
