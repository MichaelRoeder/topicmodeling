...
public class TopIntShortCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopIntShortCollectionDescending() {
        ...
        TopIntShortCollection topCollection = new TopIntShortCollection(TOP_X, false);
    ...
    @Test
    public void testTopIntShortCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        int values[] = new int[DATA_SIZE];
        short objects[] = new short[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopIntShortCollection topCollection = new TopIntShortCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
