...
public class TopByteCharCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopByteCharCollectionDescending() {
        ...
        TopByteCharCollection topCollection = new TopByteCharCollection(TOP_X, false);
    ...
    @Test
    public void testTopByteCharCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        byte values[] = new byte[DATA_SIZE];
        char objects[] = new char[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopByteCharCollection topCollection = new TopByteCharCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
