...
public class TopFloatIntCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopFloatIntCollectionDescending() {
        ...
        TopFloatIntCollection topCollection = new TopFloatIntCollection(TOP_X, false);
    ...
    @Test
    public void testTopFloatIntCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        float values[] = new float[DATA_SIZE];
        int objects[] = new int[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopFloatIntCollection topCollection = new TopFloatIntCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
