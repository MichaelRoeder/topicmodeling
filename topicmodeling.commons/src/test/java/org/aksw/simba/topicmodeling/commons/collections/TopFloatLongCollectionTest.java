...
public class TopFloatLongCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopFloatLongCollectionDescending() {
        ...
        TopFloatLongCollection topCollection = new TopFloatLongCollection(TOP_X, false);
    ...
    @Test
    public void testTopFloatLongCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        float values[] = new float[DATA_SIZE];
        long objects[] = new long[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopFloatLongCollection topCollection = new TopFloatLongCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
