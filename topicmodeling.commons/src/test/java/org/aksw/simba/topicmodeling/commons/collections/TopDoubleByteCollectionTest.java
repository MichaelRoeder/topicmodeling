...
public class TopDoubleByteCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopDoubleByteCollectionDescending() {
        ...
        TopDoubleByteCollection topCollection = new TopDoubleByteCollection(TOP_X, false);
    ...
    @Test
    public void testTopDoubleByteCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        double values[] = new double[DATA_SIZE];
        byte objects[] = new byte[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopDoubleByteCollection topCollection = new TopDoubleByteCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
