...
public class TopShortObjectCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopShortObjectCollectionDescending() {
        ...
        TopShortObjectCollection<Integer> topCollection = new TopShortObjectCollection<Integer>(TOP_X, false);
    ...
    @Test
    public void testTopShortObjectCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        Integer objects[] = new Integer[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopShortObjectCollection<Integer> topCollection = new TopShortObjectCollection<Integer>(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
