...
public class TopFloatByteCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopFloatByteCollectionDescending() {
        ...
        TopFloatByteCollection topCollection = new TopFloatByteCollection(TOP_X, false);
    ...
    @Test
    public void testTopFloatByteCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        float values[] = new float[DATA_SIZE];
        byte objects[] = new byte[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopFloatByteCollection topCollection = new TopFloatByteCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
