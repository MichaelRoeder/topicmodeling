...
public class TopIntByteCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopIntByteCollectionDescending() {
        ...
        TopIntByteCollection topCollection = new TopIntByteCollection(TOP_X, false);
    ...
    @Test
    public void testTopIntByteCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        int values[] = new int[DATA_SIZE];
        byte objects[] = new byte[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopIntByteCollection topCollection = new TopIntByteCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
