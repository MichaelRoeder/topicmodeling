...
public class TopShortByteCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopShortByteCollectionDescending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        byte objects[] = new byte[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
            values[i] = (short)(rand.nextInt() >> 16);
            objects[i] = (byte)(rand.nextInt() >> 24);
        }

        TopShortByteCollection topCollection = new TopShortByteCollection(TOP_X, false);
        ...
        int start = 0;
        int end = 1;
        byte objectsSubArray1[], objectsSubArray2[];
        byte collectionObjects[] = topCollection.getObjects();
    ...
    @Test
    public void testTopShortByteCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        byte objects[] = new byte[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopShortByteCollection topCollection = new TopShortByteCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
