...
public class TopCharByteCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopCharByteCollectionDescending() {
        ...
        TopCharByteCollection topCollection = new TopCharByteCollection(TOP_X, false);
    ...
    @Test
    public void testTopCharByteCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        char values[] = new char[DATA_SIZE];
        byte objects[] = new byte[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopCharByteCollection topCollection = new TopCharByteCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
