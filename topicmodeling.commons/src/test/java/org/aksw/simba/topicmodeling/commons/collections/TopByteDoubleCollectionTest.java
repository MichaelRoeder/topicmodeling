...
public class TopByteDoubleCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopByteDoubleCollectionDescending() {
        ...
        TopByteDoubleCollection topCollection = new TopByteDoubleCollection(TOP_X, false);
    ...
    @Test
    public void testTopByteDoubleCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        byte values[] = new byte[DATA_SIZE];
        double objects[] = new double[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopByteDoubleCollection topCollection = new TopByteDoubleCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
