...
public class TopShortIntCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopShortIntCollectionDescending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        int objects[] = new int[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
            values[i] = (short)(rand.nextInt() >> 16);
            objects[i] = rand.nextInt();
        }

        TopShortIntCollection topCollection = new TopShortIntCollection(TOP_X, false);
        ...
        int start = 0;
        int end = 1;
        int objectsSubArray1[], objectsSubArray2[];
        int collectionObjects[] = topCollection.getObjects();
    ...
    @Test
    public void testTopShortIntCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        int objects[] = new int[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopShortIntCollection topCollection = new TopShortIntCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
