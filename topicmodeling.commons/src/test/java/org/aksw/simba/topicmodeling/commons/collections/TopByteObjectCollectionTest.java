...
public class TopByteObjectCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopByteObjectCollectionDescending() {
        ...
        TopByteObjectCollection<Integer> topCollection = new TopByteObjectCollection<Integer>(TOP_X, false);
    ...
    @Test
    public void testTopByteObjectCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        byte values[] = new byte[DATA_SIZE];
        Integer objects[] = new Integer[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopByteObjectCollection<Integer> topCollection = new TopByteObjectCollection<Integer>(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
