...
public class TopByteBooleanCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopByteBooleanCollectionDescending() {
        ...
        TopByteBooleanCollection topCollection = new TopByteBooleanCollection(TOP_X, false);
    ...
    @Test
    public void testTopByteBooleanCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        byte values[] = new byte[DATA_SIZE];
        boolean objects[] = new boolean[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopByteBooleanCollection topCollection = new TopByteBooleanCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
