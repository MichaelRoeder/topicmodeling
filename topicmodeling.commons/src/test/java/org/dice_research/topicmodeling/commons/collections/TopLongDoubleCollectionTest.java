...
public class TopLongDoubleCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopLongDoubleCollectionDescending() {
        ...
        TopLongDoubleCollection topCollection = new TopLongDoubleCollection(TOP_X, false);
    ...
    @Test
    public void testTopLongDoubleCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        long values[] = new long[DATA_SIZE];
        double objects[] = new double[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopLongDoubleCollection topCollection = new TopLongDoubleCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
