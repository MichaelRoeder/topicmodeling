...
public class TopLongIntCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopLongIntCollectionDescending() {
        ...
        TopLongIntCollection topCollection = new TopLongIntCollection(TOP_X, false);
    ...
    @Test
    public void testTopLongIntCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        long values[] = new long[DATA_SIZE];
        int objects[] = new int[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopLongIntCollection topCollection = new TopLongIntCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
