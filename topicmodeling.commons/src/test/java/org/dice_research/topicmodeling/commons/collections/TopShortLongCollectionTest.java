...
public class TopShortLongCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopShortLongCollectionDescending() {
        ...
        TopShortLongCollection topCollection = new TopShortLongCollection(TOP_X, false);
    ...
    @Test
    public void testTopShortLongCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        long objects[] = new long[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopShortLongCollection topCollection = new TopShortLongCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
