...
public class TopByteIntCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopByteIntCollectionDescending() {
        ...
        TopByteIntCollection topCollection = new TopByteIntCollection(TOP_X, false);
    ...
    @Test
    public void testTopByteIntCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        byte values[] = new byte[DATA_SIZE];
        int objects[] = new int[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopByteIntCollection topCollection = new TopByteIntCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
