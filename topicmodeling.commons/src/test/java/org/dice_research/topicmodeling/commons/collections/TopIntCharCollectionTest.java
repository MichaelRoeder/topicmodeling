...
public class TopIntCharCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopIntCharCollectionDescending() {
        ...
        TopIntCharCollection topCollection = new TopIntCharCollection(TOP_X, false);
    ...
    @Test
    public void testTopIntCharCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        int values[] = new int[DATA_SIZE];
        char objects[] = new char[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopIntCharCollection topCollection = new TopIntCharCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
