...
public class TopIntFloatCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopIntFloatCollectionDescending() {
        ...
        TopIntFloatCollection topCollection = new TopIntFloatCollection(TOP_X, false);
    ...
    @Test
    public void testTopIntFloatCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        int values[] = new int[DATA_SIZE];
        float objects[] = new float[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopIntFloatCollection topCollection = new TopIntFloatCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
