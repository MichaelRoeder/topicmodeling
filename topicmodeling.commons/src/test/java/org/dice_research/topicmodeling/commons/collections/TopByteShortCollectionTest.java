...
public class TopByteShortCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopByteShortCollectionDescending() {
        ...
        TopByteShortCollection topCollection = new TopByteShortCollection(TOP_X, false);
    ...
    @Test
    public void testTopByteShortCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        byte values[] = new byte[DATA_SIZE];
        short objects[] = new short[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopByteShortCollection topCollection = new TopByteShortCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
