...
public class TopShortCharCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopShortCharCollectionDescending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        char objects[] = new char[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
            values[i] = (short)(rand.nextInt() >> 16);
            objects[i] = (char) rand.nextInt();
        }

        TopShortCharCollection topCollection = new TopShortCharCollection(TOP_X, false);
        ...
        int start = 0;
        int end = 1;
        char objectsSubArray1[], objectsSubArray2[];
        char collectionObjects[] = topCollection.getObjects();
    ...
    @Test
    public void testTopShortCharCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        char objects[] = new char[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopShortCharCollection topCollection = new TopShortCharCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
