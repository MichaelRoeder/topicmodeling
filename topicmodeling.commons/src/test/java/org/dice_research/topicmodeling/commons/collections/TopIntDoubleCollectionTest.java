...
public class TopIntDoubleCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopIntDoubleCollectionDescending() {
        ...
        TopIntDoubleCollection topCollection = new TopIntDoubleCollection(TOP_X, false);
    ...
    @Test
    public void testTopIntDoubleCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        int values[] = new int[DATA_SIZE];
        double objects[] = new double[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopIntDoubleCollection topCollection = new TopIntDoubleCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
