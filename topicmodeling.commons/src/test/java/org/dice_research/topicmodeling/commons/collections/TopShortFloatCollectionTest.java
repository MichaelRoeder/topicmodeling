...
public class TopShortFloatCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopShortFloatCollectionDescending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        float objects[] = new float[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
            values[i] = (short)(rand.nextInt() >> 16);
            objects[i] = rand.nextFloat();
        }

        TopShortFloatCollection topCollection = new TopShortFloatCollection(TOP_X, false);
        ...
        int start = 0;
        int end = 1;
        float objectsSubArray1[], objectsSubArray2[];
        float collectionObjects[] = topCollection.getObjects();
    ...
    @Test
    public void testTopShortFloatCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        short values[] = new short[DATA_SIZE];
        float objects[] = new float[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopShortFloatCollection topCollection = new TopShortFloatCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
