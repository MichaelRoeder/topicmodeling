...
public class TopIntLongCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopIntLongCollectionDescending() {
        ...
        TopIntLongCollection topCollection = new TopIntLongCollection(TOP_X, false);
    ...
    @Test
    public void testTopIntLongCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        int values[] = new int[DATA_SIZE];
        long objects[] = new long[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopIntLongCollection topCollection = new TopIntLongCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
