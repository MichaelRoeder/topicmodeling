...
public class TopLongCharCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopLongCharCollectionDescending() {
        ...
        TopLongCharCollection topCollection = new TopLongCharCollection(TOP_X, false);
    ...
    @Test
    public void testTopLongCharCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        long values[] = new long[DATA_SIZE];
        char objects[] = new char[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopLongCharCollection topCollection = new TopLongCharCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
