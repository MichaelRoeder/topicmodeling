...
public class TopFloatFloatCollectionTest {

    private static final int DATA_SIZE = 4096;
    private static final int TOP_X = 100;

    @Test
    public void testTopFloatFloatCollectionDescending() {
        ...
        TopFloatFloatCollection topCollection = new TopFloatFloatCollection(TOP_X, false);
    ...
    @Test
    public void testTopFloatFloatCollectionAscending() {
        Random rand = new Random(System.currentTimeMillis());
        float values[] = new float[DATA_SIZE];
        float objects[] = new float[DATA_SIZE];
        for (int i = 0; i < DATA_SIZE; ++i) {
        ...
        TopFloatFloatCollection topCollection = new TopFloatFloatCollection(TOP_X, true);
        for (int i = 0; i < values.length; i++) {
            topCollection.add(values[i], objects[i]);
        }
