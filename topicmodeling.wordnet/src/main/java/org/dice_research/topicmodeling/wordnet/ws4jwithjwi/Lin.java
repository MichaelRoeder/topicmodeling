...
public class Lin extends AbstractRelatednessCalculator {
    ...
    public Lin(IDictionary dictionary) {
        super(dictionary);
    ...
    public double calcRelatednessOfSynsets(ISynset synset1,
        ...
        double icSynset1 = icTree.getInformationContent(synset1.getID());
        double icSynset2 = icTree.getInformationContent(synset2.getID());
        double icDcs = 0;
        if (dcs.getCommonSynset() != null) {
            icDcs = icTree.getInformationContent(dcs.getCommonSynset().getID());
        }
        double score = (double) (2 * icDcs)
                / (double) (icSynset1 + icSynset2);
        return score;
    }
